...
.. list-table::
   ...
   * - Class / Method
     - Description
   * - ``RsyncClient(hostname, username, ...)``
     - Create an rsync wrapper with SSH connection parameters
   * - ``RsyncClient.push(local, remote, ...)``
     - Sync local to remote (``--delete`` by default)
   * - ``RsyncClient.pull(remote, local, ...)``
     - Sync remote to local (no ``--delete`` by default)
   * - ``RsyncClient.get_default_remote_path()``
     - Returns ``~/.config/srunx/workspace/{repo_name}/``
   * - ``RsyncResult.success``
     - ``True`` if rsync exited with code 0
