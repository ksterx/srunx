...
``srunx.config`` refer to the SAME module object as
``srunx.common.config`` via a ``sys.modules`` alias — so attribute
access, monkey-patching of cached state (e.g. ``srunx.config._config =
...
statically and needs to see that ``srunx.config.SrunxConfig`` resolves.
"""
